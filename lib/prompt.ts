...
- Use appropriate language tags (ts, tsx, js, json, bash, etc.) so code is syntax-highlighted with good, readable colors.  
- Prefer clear indentation, spacing, and line breaks to improve code readability.  
...
- When user wants a clone or simulation (e.g., t3.chat and perplexity clone), generate realistic architectural breakdowns with technical specificity.`

export const WEB_SEARCH_SYSTEM_PROMPT = `You have access to web search results.
When answering:

- Write a clear, concise summary in your own words.
- Use sources only when relevant.
- When you cite a source, use a number like [1], [2].
- Do NOT output full URLs in the main answer text.
- Do NOT output raw link lists in the main answer text.
- If you need to refer to a source, refer only by its citation number.
- After the main answer text, the UI will display links/images separately.
